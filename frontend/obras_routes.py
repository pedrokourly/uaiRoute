...
def cadastrarObra():
    if request.method == 'POST':
        ...
        try:
            response = backend_client.post(API_URLS['obras'], json=obra)
...
def editarObra(id):
    if request.method == 'POST':
        ...
        try:
            response = backend_client.put(f'{API_URLS["obras"]}{id}/', json=obra)
...
def excluirObra(id):
    try:
        response = backend_client.delete(f'{API_URLS["obras"]}{id}/')
        if response.status_code in [200, 204]:
            return redirect(url_for('obras'))
        else:
            return redirect(url_for('obras'))
